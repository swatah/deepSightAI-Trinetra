...
### Actual results (single-instance run, 2026-09-18)
...
### Actual results (two-extractor/two-embedder run, 2026-09-18)

Same 3 videos, same queries, this time against 2 extractors + 2 embedders on a fresh VM (RTX 3060) — testing the registry atomicity fix and the config-driven concurrency limits under real (if sequential) multi-instance traffic. All 3 videos ingested and embedded successfully: 48 frames (cam_01, 9.6s × 5fps), 99 frames (cam_02, 19.8s × 5fps), 45 frames (cam_03, 9.0s × 5fps) — exact frame counts confirm 5fps sampling is working correctly end-to-end.

Round-robin confirmed working across multiple instances: extraction jobs split 2/2 across `extractor-1`/`extractor-2`, embedding completions split 2/1 across `embedder-1`/`embedder-2` — no double-assignment, no crashes, matching what the earlier Lua-script concurrency stress test predicted.

Search results held up the same as the single-instance run: `"a car driving on the street"` (top_k=30) returned a `cam_02_street_crossing`-heavy but multi-camera result set; both pedestrian-flavored queries again correctly top-ranked `cam_02_street_crossing` (0.353 and 0.340 respectively).

This run surfaced **four more real bugs**, none related to today's own changes except that this was the first time the event-driven embedder path was ever actually exercised live — see §9:
1. `Embedder/requirements_embedder.txt` was missing `redis` and `pydantic`.
2. `shared/streaming/consumer.py` called `xgroup_create(stream=...)`, which current redis-py rejects — the kwarg is `name=`.
3. A client-side socket timeout on a blocking `XREADGROUP` was treated as a fatal consumer error instead of "no messages this window."
4. **The big one**: `extractor.py` published `FrameReadyEvent`s to a Redis stream literally named `"frames"`, while `embedder.py`'s consumer read from `"events:frame_ready"` — the two sides of the event pipeline were never actually connected, silently, until this run. Fixed the stream name. Separately, `embedder.py`'s event handler itself referenced fields (`segment_path`, `rtsp_bucket`, `event_type == "frame_ready"`) that don't exist on the actual `FrameReadyEvent` schema (which has `segment_id`, `frame_paths`, `bucket_name`, `event_type == "frame.ready"`) — fixed to use the real schema and the event's own `frame_paths` directly instead of re-listing MinIO.

Also worth noting: this run's first VM (a GTX 1070) turned out to have a GPU too old for the pinned PyTorch's CUDA build (`sm_61`, unsupported) — destroyed and re-provisioned with a compute-capability filter (`compute_cap>=750`) instead of picking on price alone. The replacement VM's driver (CUDA 12.4) was in turn too old for the *default* PyPI `torch` install (which resolves to a `cu130` build) — fixed by explicitly installing `torch==2.6.0+cu124` / `torchvision==0.21.0+cu124` from PyTorch's own index in both the embedder and SearchService venvs. Worth checking both GPU architecture *and* host driver version against whatever torch build gets installed, not just one or the other.
...
- **The extractor and embedder were never actually connected to each other, silently.** `extractor.py`'s `publish_frame_ready_event()` published to a Redis stream literally named `"frames"`; `embedder.py`'s consumer group read from `"events:frame_ready"`. Different names, so every published event just sat in the wrong stream forever — no error anywhere, extraction and MinIO upload both looked completely successful, the embedder just never heard about any of it. This had never been caught before because the two-extractor/two-embedder run on 2026-09-18 was the first time this repo's event-driven embedder path (added in an earlier commit, `dd66b87`) was ever exercised end-to-end on real infrastructure. **Fixed** by publishing to `"events:frame_ready"` to match what the consumer actually reads.
- **Even with the stream name fixed, `embedder.py`'s event handler itself didn't match the real event schema.** It read `event.segment_path`, `event.rtsp_bucket`, and checked `event.event_type == "frame_ready"` — none of which exist on the actual `FrameReadyEvent` model (`shared/streaming/schema.py`), which has `segment_id` (int), `frame_paths` (list), `bucket_name`, and `event_type == "frame.ready"` (dot, not underscore). Every event was silently dropped with `AttributeError`, never acked, so it'd sit as a permanently-pending message. **Fixed** to use the real fields, and to pass the event's own `frame_paths` directly into `process_segment_frames`/`process_rtsp_frames` instead of re-listing the bucket from MinIO (the event already tells you exactly which frames are ready).
- **`Embedder/requirements_embedder.txt` was missing `redis` and `pydantic`**, even though `shared.streaming.consumer`/`schema` (imported by `embedder.py`) require both. Never caught before for the same reason as the stream-name bug above — this code path had never actually run. **Fixed** by adding both.
- **`shared/streaming/consumer.py` called `xgroup_create(stream=stream_name, ...)`** — current redis-py (8.1.0) doesn't accept a `stream` keyword there; the parameter is `name`. **Fixed**.
- **A blocking `XREADGROUP` read hitting the client's own socket timeout was treated as a fatal consumer error**, not as "no new messages in this window" (which is what it actually means for an intentional blocking poll). This put the embedder into an error/backoff loop on every quiet 5-second window. **Fixed** by catching `redis.exceptions.TimeoutError` in `StreamConsumer.read()` and returning an empty list.
...
- **On this vast.ai image, the conda-installed `ffmpeg` (first on `PATH`) silently lacks `libx264`** — it's a GPL-avoidance build (`conda-bld/ffmpeg`, no `--enable-libx264` in its config), while the apt-installed `/usr/bin/ffmpeg` has it. `ffmpeg-python`'s `.run()` just shells out to whatever `ffmpeg` resolves to on `PATH`, so segment extraction failed with `Unknown encoder 'libx264'` even though a working `ffmpeg` was installed and present on the box the whole time. §4 Step 2 fixes this by symlinking `/opt/conda/bin/ffmpeg` to the system one, so `PATH` order stops mattering. This has to be redone on every fresh VM — it's a per-instance symlink, not something fixed in the repo.
- **GPU choice matters in two independent ways, not just price.** First pass at picking a vast.ai offer for the two-extractor/two-embedder run went by cheapest `dph_total` alone and landed on a GTX 1070 — a 2016 (Pascal) card whose compute capability (`sm_61`) current PyTorch CUDA wheels no longer ship kernels for; the model "loaded" but was silently unusable on GPU. Re-provisioned with a `compute_cap>=750` filter on the offer search instead of sorting by price alone, which picked an RTX 3060 (Ampere, `sm_86`) — but that VM's **driver** version (550.120, CUDA 12.4) was in turn too old for the *default* `pip install torch`, which resolves to the newest build (`cu130`) regardless of what the host can actually run; `torch.cuda.is_available()` silently returned `False` with only a buried `UserWarning` in the logs. Fixed by explicitly installing `torch==2.6.0+cu124`/`torchvision==0.21.0+cu124` from PyTorch's own package index (`--index-url https://download.pytorch.org/whl/cu124 --extra-index-url https://pypi.org/simple` — the `--extra-index-url` is needed too, or pip can't resolve build-only dependencies like `flit_core` that the cu124 index doesn't carry). Both the GPU's own architecture *and* the host's installed driver version need checking against whatever torch build actually gets installed — matching one but not the other still silently falls back to CPU or breaks at import time.
